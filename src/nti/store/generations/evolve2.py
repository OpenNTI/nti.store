...
def _hard_removal(index, iid, intids):
    ...
    try:
        ...
        if IPurchaseAttempt.providedBy(pa):
            intids.force_unregister(
                iid, pa, notify=False, remove_attribute=False)
            result = True
    except Exception as e:
        logger.warn(
            "After hard removal; possibly leaking int-id %s. %s", iid, e)
...
def update_user_purchase_data(user, intids=None):
    ...
    logger.debug(
        "%s updated=%s, removed=%s", user, update_count, removed_count)
    return (update_count, removed_count)
