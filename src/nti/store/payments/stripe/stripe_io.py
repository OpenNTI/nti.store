...
def create_stripe_charge(amount, currency='USD', customer_id=None, card=None,
    ...
    charge = _do_stripe_operation(
        stripe.Charge.create, api_key=api_key, **data
    )
    return charge
...
def get_stripe_charge(charge_id, api_key=None):
    result = _do_stripe_operation(
        stripe.Charge.retrieve, charge_id, api_key=api_key)
    return result
...
def get_stripe_coupon(coupon_code, api_key=None):
    coupon = _do_stripe_operation(
        stripe.Coupon.retrieve, coupon_code, api_key=api_key)
    return coupon
